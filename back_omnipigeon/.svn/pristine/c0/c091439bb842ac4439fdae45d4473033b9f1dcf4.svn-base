...
public class OrganisationController extends AbstractController {
    ...
    public List<OrganisationDto> getStations(HttpServletRequest request, HttpServletResponse response) throws OmniException {
     ...
     @RequestMapping(value = "/organisationById", method = {RequestMethod.GET, RequestMethod.OPTIONS})
    @ResponseBody
//    @JsonView({OmniJsonView.ClientView.class})
    public OrganisationDto getOrganisationById(
            @RequestParam(value = "organisationId") Long organisationId, HttpServletRequest request,
            HttpServletResponse response
    ) throws OmniException {
        return mapper.map(organisationService.findOrganisationById(organisationId), OrganisationDto.class);
    }
